"""SCI Fig 7 v2: Fix panel C legend (show all conc), remove ambiguous labels"""
import os, sys
...
gs = GridSpec(2, 3, width_ratios=[1.3, 1.0, 1.0], hspace=0.55, wspace=0.45)
...
# C: cycle stability (show ALL conc in legend)
csv = os.path.join(DOWN, 'cycle_stability.csv')
axC = fig.add_subplot(gs[0, 2])
if os.path.exists(csv):
    cd = pd.read_csv(csv)
    # Get unique conc pairs per material
    for (mat, conc1), g in cd.groupby(['material', 'conc1']):
        ...
        if 1 in pred and 3 in pred:
            ...
            # ALWAYS add to legend (not just selected conc)
    ...
    # Legend OUTSIDE right (avoid data overlap)
...
# D: per-strain residual ridge plot (replaces stripplot)
...
# Jittered scatter by material with median line
for mat, color, name in [(0, C_OHA, 'OHA-GEL'), (1, C_ALG, 'Alginate')]:
    mask = err_mats == mat
    vals = err_mag[mask]
